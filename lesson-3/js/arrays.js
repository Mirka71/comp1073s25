...
output.textContent = `The first of the element of array myArray is ${myArray[3][0]}`;
...
// output.textContent  `There are ${myArray.length} elements in my array.`;
output.textContent = `There are a total of ${totalThingsInArray} elements in my array.`;
output.textContent = "";
...
for (let i = 0; i < myArray.length; i++) {
    ...
    if (Array.isArray(myArray[i])) {
        ...
        for (let j = 0; j < myArray[1].length; j++) {
